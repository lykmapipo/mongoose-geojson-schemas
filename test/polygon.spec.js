...
import { Polygon, GEO_2DSPHERE } from '../src';
...
describe('Polygon', () => {
  ...
  it('should be a schema', () => {
    ...
    expect(Polygon.type.paths.type.constructor.name).to.be.equal(
      'SchemaString'
    );

    // assert coordinates
    expect(Polygon.type.paths.coordinates).exist;
    expect(Polygon.type.paths.coordinates.constructor.name).to.be.equal(
      'SchemaArray'
    );
  });
  ...
  it('indexes are created when model is compiled', (done) => {
    ...
    PLOI.on('index', () => {
      PLOI.collection.getIndexes({ full: true }, (error, indexes) => {
        // assert indexes
        ...
        const index = _.find(indexes, { key: { farm: GEO_2DSPHERE } });
  ...
  it('should be able to validate - invalid', (done) => {
    ...
    ploi.validate((error) => {
      ...
      expect(error.errors.farm.message).to.be.equal(
        'farm is not a valid GeoJSON Polygon'
      );
      done();
    });
  });
