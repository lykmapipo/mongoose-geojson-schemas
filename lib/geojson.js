...
/**
 * @name fromString
 * @description create geojson geometry from string
 * @param {String} coords string to extract geojson geometry
 * @param {String} [delimiter=','] long, lat seperator from string
 * @param {String} [separator=' '] long, lat pair seperator from string
 * @return {Geometry} random geojson multipolygon(s)
 */
exports.fromString = function (coords = '', deliminator = ',', separator = ' ') {
  ...
  try {
    if (!_.isEmpty(coords)) {
      ...
      const points = _.map(pairs, function (pair) {
      ...
      // parse a multipoint
      if (isMultiPointCoor(points) && !isPolygonCoor([points])) {
        const { geometry } = turf.multiPoint(points);
        return geometry;
      }
    }
  } catch (error) {}
